...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockRepository.saveEquipmentCheck(any()))
        .thenAnswer((_) async => {});
  });
  ...
  group('EquipmentCheckFormScreen Widget Tests', () {
    testWidgets('should render equipment tabs, check type toggle, summary badge, and SOP cards', (tester) async {
    ...
    testWidgets('should update SOP checklist items when switching equipment type tabs', (tester) async {
      await tester.pumpWidget(buildTestWidget());
      await tester.pumpAndSettle();
    ...
    testWidgets('should update status to FLAGGED when an SOP item is marked FAIL', (tester) async {
      await tester.pumpWidget(buildTestWidget());
      await tester.pumpAndSettle();
      ...
      expect(find.text('Catatan Kerusakan / Kendala (Wajib)'), findsOneWidget);
    });

    testWidgets('should save equipment check when submit button is pressed', (tester) async {
      await tester.pumpWidget(buildTestWidget());
      await tester.pumpAndSettle();

      final serialNumberField = find.widgetWithText(TextField, 'Nomor Seri Alat / ID Unit');
      expect(serialNumberField, findsOneWidget);

      await tester.enterText(serialNumberField, 'GNSS-TEST-99');
      await tester.pumpAndSettle();

      final submitButton = find.widgetWithText(ElevatedButton, 'Simpan Inspeksi SOP (5/5 Lolos)');
      ...
      expect(find.text('Pemeriksaan SOP berhasil disimpan offline'), findsOneWidget);
    });
  });
}
