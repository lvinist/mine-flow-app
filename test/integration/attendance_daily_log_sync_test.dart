...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('attendance_daily_log_sync_test_');
    Hive.init(tempDir.path);

    attendanceBox = await Hive.openBox<AttendanceRecordDto>('test_attendance_box');
  ...
  group('Attendance & Daily Log Offline Sync Integration Tests', () {
    test('Attendance offline creation enqueues mutation and flushes when online', () async {
      ...
      syncQueueManager.registerEntityHandler('attendance_records', (item) async {
      ...
      final localRecord = await attendanceRepository.getAttendanceById('att-sync-101');
      ...
      expect(syncedAttendancePayloads.first['user_id'], equals('user-miner-01'));
    ...
    test('Daily Log draft auto-save and submission offline queueing and sync', () async {
      fakeNetworkInfo.setConnected(false);
      ...
      final submittedLog = await dailyLogRepository.getDailyLogById('log-sync-202');
      expect(submittedLog, isNotNull);
      expect(submittedLog!.status, equals(LogStatus.submitted));
    });

    test('Timestamp conflict resolution during offline queue sync processing', () async {
      fakeNetworkInfo.setConnected(false);
