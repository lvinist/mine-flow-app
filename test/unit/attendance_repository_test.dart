...
void main() {
  ...
  setUpAll(() async {
    ...
    if (!Hive.isAdapterRegistered(21)) Hive.registerAdapter(AttendanceRecordDtoAdapter());
  });

  setUp(() async {
    attendanceBox = await Hive.openBox<AttendanceRecordDto>('test_attendance_box_${DateTime.now().millisecondsSinceEpoch}');
  ...
  group('AttendanceRepositoryImpl - Offline First Operations', () {
    ...
    test('saveAttendance should write to local cache and enqueue sync mutation item', () async {
    ...
    test('saveAttendanceBatch should save multiple attendance records to local cache', () async {
    ...
    test('getAttendanceForDate should filter local cache by date and siteId', () async {
      await repository.saveAttendanceBatch([tRecord1, tRecord2]);
    ...
    test('deleteAttendance should mark soft delete in local cache and enqueue delete mutation', () async {
      await repository.saveAttendance(tRecord1);
      ...
      final recordsForDate = await repository.getAttendanceForDate(DateTime(2026, 7, 18));
