...
class MockEquipmentCheckRemoteDataSource implements EquipmentCheckRemoteDataSource {
...
void main() {
  ...
  setUpAll(() async {
    ...
    if (!Hive.isAdapterRegistered(23)) Hive.registerAdapter(EquipmentCheckDtoAdapter());
  });
  ...
  group('EquipmentCheckRepositoryImpl - Offline First Operations', () {
    ...
    final tCheck2 = EquipmentCheck(
      ...
      isOperational: false,
      checklist: const [
        CheckItem(id: 'prop_damaged', label: 'Propeller Check', isPassed: false),
      ],
      remarks: 'Damaged propeller blade',
    );

    test('saveEquipmentCheck should write to local cache and enqueue sync mutation item', () async {
    ...
    test('saveEquipmentCheckBatch should save multiple checks to local cache', () async {
    ...
    test('getEquipmentChecks should filter local cache by equipmentType and siteId', () async {
      await repository.saveEquipmentCheckBatch([tCheck1, tCheck2]);
    ...
    test('getEquipmentCheckById should return specific equipment check', () async {
      await repository.saveEquipmentCheck(tCheck1);
    ...
    test('deleteEquipmentCheck should mark soft delete in local cache and enqueue delete mutation', () async {
      await repository.saveEquipmentCheck(tCheck1);
    ...
    test('syncRemote should fetch remote records and update local cache when online', () async {
      ...
      expect(syncedChecks.first.equipmentType, equals(EquipmentType.totalStation));
