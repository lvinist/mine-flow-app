...
void main() {
  ...
  setUpAll(() async {
    ...
    if (!Hive.isAdapterRegistered(10)) Hive.registerAdapter(SyncQueueItemAdapter());
    if (!Hive.isAdapterRegistered(11)) Hive.registerAdapter(SyncActionAdapter());
    if (!Hive.isAdapterRegistered(12)) Hive.registerAdapter(SyncStatusAdapter());
    if (!Hive.isAdapterRegistered(22)) Hive.registerAdapter(DailyLogDtoAdapter());
  });

  setUp(() async {
    dailyLogBox = await Hive.openBox<DailyLogDto>('test_daily_log_box_${DateTime.now().millisecondsSinceEpoch}');
    queueBox = await Hive.openBox<SyncQueueItem>('test_queue_box_${DateTime.now().millisecondsSinceEpoch}');
  ...
  group('DailyLogRepositoryImpl - Offline First Operations', () {
    ...
    test('autoSaveDraft should store log as draft in local cache and enqueue sync mutation', () async {
    ...
    test('getDraftLogForForeman should locate existing draft log for foreman on specified date', () async {
      await repository.autoSaveDraft(tLog1);
    ...
    test('submitDailyLog should transition status to submitted and update timestamp', () async {
      await repository.autoSaveDraft(tLog1);
    ...
    test('approveDailyLog should set status to approved and store approvedBy supervisor ID', () async {
      await repository.autoSaveDraft(tLog2);

      await repository.approveDailyLog('log-002', approvedBy: 'supervisor-99');
      ...
      final pendingQueue = queueRepo.getAll();
      expect(pendingQueue.last.payloadJson['approved_by'], equals('supervisor-99'));
    });

    test('getDailyLogs should filter correctly by date, foremanId, zoneId, and status', () async {
      await repository.autoSaveDraft(tLog1);
      await repository.autoSaveDraft(tLog2);

      final allToday = await repository.getDailyLogs(date: DateTime(2026, 7, 18));
      expect(allToday.length, equals(2));

      final foreman1Logs = await repository.getDailyLogs(foremanId: 'foreman-1');
    ...
    test('deleteDailyLog should mark soft delete in local cache and enqueue delete mutation', () async {
      await repository.autoSaveDraft(tLog1);
      ...
      final logsToday = await repository.getDailyLogs(date: DateTime(2026, 7, 18));
      expect(logsToday.isEmpty, isTrue);

      final queueItems = queueRepo.getAll();
      final hasDeleteMutation = queueItems.any((item) => item.action == SyncAction.delete);
      expect(hasDeleteMutation, isTrue);
    });

    test('syncRemote should fetch remote logs and populate local cache when online', () async {
