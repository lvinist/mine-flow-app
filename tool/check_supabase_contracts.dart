...
void main() {
  ...
  print(
    '  supabase gen types dart --project-id \$SUPABASE_PROJECT_ID > $generatedFilePath\n',
  );

  if (!generatedFile.existsSync()) {
    print(
      '[ERROR] The generated types file ($generatedFilePath) does not exist.',
    );
    exit(1);
  }
  ...
  if (migUncommitted && !typeUncommitted) {
    print(
      '[ERROR] Uncommitted database migrations found, but $generatedFilePath is not updated.',
    );
  ...
  if (isCi) {
    ...
    if (migChanged && !typeChanged) {
      print(
        '[ERROR] Migrations modified in this PR/push, but $generatedFilePath was not updated.',
      );
